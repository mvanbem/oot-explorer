    ...
    pub fn get_virtual_slice(
        ...
        match self.virtual_starts.binary_search(&range.start) {
            ...
            Err(next_index) => {
                ...
                let file = self.get_file(scope, index);

                let file_virtual_start = self.virtual_starts[index];
                let start_within_file = (range.start - file_virtual_start) as usize;
                let end_within_file = (range.end - file_virtual_start) as usize;
                &file[start_within_file..end_within_file]
            }
        }
    }
}
