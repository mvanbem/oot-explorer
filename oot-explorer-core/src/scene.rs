...
use crate::header::scene::SCENE_HEADER_DESC;
...
const SCENE_INNER_DESC: &StructDescriptor = &StructDescriptor {
    name: "Scene",
    size: None,
    is_end: None,
    fields: &[FieldDescriptor {
        name: "headers",
        location: StructFieldLocation::InlineDelimitedList { offset: 0 },
        desc: SCENE_HEADER_DESC,
    }],
};

pub const SCENE_DESC: TypeDescriptor = TypeDescriptor::Struct(SCENE_INNER_DESC);
...
impl<'a> Scene<'a> {
    ...
    pub fn headers(self) -> impl Iterator<Item = SceneHeaderVariant<'a>> {
        SceneHeaderIter::new(self.data).flat_map(|header| header.variant().ok())
    }
}
