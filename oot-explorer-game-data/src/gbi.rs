...
use oot_explorer_read::{aligned_data, is_end, FromVrom, Layout, ReadError, Sentinel};
use oot_explorer_reflect::{StructDescriptor, TypeDescriptor, I16_DESC, I8_DESC, U8_DESC};
...
compile_interfaces! {
    #[layout(size = 0, align_bits = 2)]
    struct DisplayList {
        struct Instruction[..] instructions @0;
    }
}

pub const INSTRUCTION_DESC: TypeDescriptor = TypeDescriptor::Struct(&StructDescriptor {
    name: "Instruction",
    size: Some(8),
    is_end: Some(is_end::<Instruction>),
    fields: &[],
});
        ...
        match data[0] {
            ...
            0x01 => {
                ...
                Instruction::Vtx {
                    ...
                    segment_addr: SegmentAddr(u32_b),
                }
            }
...
impl Layout for Instruction {
    const SIZE: u32 = 8;
    const ALIGN_BITS: u32 = 2;
}

impl FromVrom for Instruction {
    fn from_vrom(vrom: Vrom<'_>, addr: VromAddr) -> Result<Self, ReadError> {
        Ok(Self::parse(aligned_data::<Self>(vrom, addr)?))
    }
}

impl Sentinel for Instruction {
    const ITER_YIELDS_SENTINEL_VALUE: bool = false;

    fn is_end(&self, _vrom: Vrom<'_>) -> bool {
        match self {
            Instruction::Dl { jump: true, .. } | Instruction::EndDl => true,
            _ => false,
        }
    }
}
...
impl UnlitVertex {
    pub fn position(self, vrom: Vrom<'_>) -> [i16; 3] {
        [
            self.position_x(vrom),
            self.position_y(vrom),
            self.position_z(vrom),
        ]
    }

    pub fn texcoord(self, vrom: Vrom<'_>) -> [i16; 2] {
        [self.texcoord_s(vrom), self.texcoord_t(vrom)]
    }

    pub fn color(self, vrom: Vrom<'_>) -> [u8; 4] {
        [
            self.color_r(vrom),
            self.color_g(vrom),
            self.color_b(vrom),
...
impl LitVertex {
    ...
    pub fn normal(self, vrom: Vrom<'_>) -> [i8; 3] {
        [
            self.normal_x(vrom),
            self.normal_y(vrom),
            self.normal_z(vrom),
